...
int City_WriteWeatherData(City* _City, json_t* weatherJson)
{
    ...
    int result = json_dump_file(weatherJson, filename, JSON_INDENT(4) | JSON_PRESERVE_ORDER);
    if (result != 0)
    {
        printf("Failed to write weather data for City %s to file %s! Errorcode: %i\n",
...
json_t* City_GetWeatherData(City* _City)
{
	...
	snprintf(cache_filepath, sizeof(cache_filepath), "cities/%s.json", _City->name);

	struct stat filinfo;
    if (stat(cache_filepath, &filinfo) == 0) // file exists
    {
        time_t now = time(NULL);
        double age = difftime(now, filinfo.st_mtime); //st_time är last modified time av filen
        ...
        {
            printf("Cache is older than 15 minutes (age %.0f seconds), fetching new data.\n", age);
        }
    }
    else
    {
